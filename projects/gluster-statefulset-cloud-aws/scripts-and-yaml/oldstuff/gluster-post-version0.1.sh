      ...
      then
          ...
          do
              ...
              then
                ...
                then
                  ...
                  then
                    ...
                    if grep -qs '$MOUNT_BASE$VOLUME_BASE$volstart/brick$volstart' /proc/mounts
                    # if (mount | grep -q "$MOUNT_BASE$VOLUME_BASE$volstart/brick$volstart type fuse.glusterfs")
                    then
                      echo " ... ... ... It does exist, so no action needed" >> /usr/share/bin/gluster.log                     
                    else
                      MOUNT_CMD="mount -t glusterfs  $DNSHOSTPOD:$VOLUME_BASE$volstart $MOUNT_BASE$VOLUME_BASE$volstart/brick$volstart"
                  ...
                  wait
                  # now if hostname is this host, then check and create mount if it doesn't exist
                  if [ "$HOSTNAME" == "$THIS_HOST" ]
                  then
                    echo " ... ... Check to see if fuse mount exists?" >> /usr/share/bin/gluster.log
                    if grep -qs '$MOUNT_BASE$VOLUME_BASE$volstart/brick$volstart' /proc/mounts
                ...
                if [ "$HOSTNAME" == "$THIS_HOST" ]
                then
                  echo " ... ... This is OUR host" >> /usr/share/bin/gluster.log
                  echo " ... ... ... Check to see if fuse mount exists?" >> /usr/share/bin/gluster.log
                  if grep -qs '$MOUNT_BASE$VOLUME_BASE$volstart/brick$volstart' /proc/mounts
                  then
                    echo " ... ... ... It does exist, so no action needed" >> /usr/share/bin/gluster.log                    
            ...
            then
              ...
              echo " creating fuse mount..." >> /usr/share/bin/gluster.log
              if [ "$MOUNT_CMD" == "" ]
              then
                echo " ... Not our host, don't run fuse mount command" >> /usr/share/bin/gluster.log
              else
                echo " ... Running Mount Command" >> /usr/share/bin/gluster.log
                result=`eval $MOUNT_CMD`
              fi
